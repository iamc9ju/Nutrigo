...
export default function StatCard({ title, value, unit, description, icon, colorClass, children }: StatCardProps) {
    return (
        <div className="bg-white p-5 rounded-3xl shadow-[0_2px_20px_rgba(0,0,0,0.03)] border border-gray-100 flex flex-col justify-between h-full hover:shadow-lg transition-shadow duration-300">
            ...
            <div className="mb-2">
                <span className="text-3xl font-black text-[#3d3522] tracking-tight">{value}</span>
                {unit && <span className="text-gray-400 text-sm ml-1 font-medium">{unit}</span>}
            </div>
            {children}
