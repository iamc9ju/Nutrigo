...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-4 left-1/2 -translate-x-1/2 w-[95%] max-w-7xl z-50 transition-all duration-300">
      <div
        className={`backdrop-blur-xl rounded-2xl px-6 py-3 flex items-center justify-between shadow-lg transition-all duration-300 border ${
          scrolled
      ...
      >
        ...
        >
          ...
          >
            บริการโภชนาการ <span className="text-xs">▼</span>
          </Link>
          <Link
            href="#"
            className={`hover:text-[#8BC34A] transition-colors flex items-center gap-1 ${scrolled ? "text-gray-600" : ""}`}
          >
            เมนูสุขภาพ <span className="text-xs">▼</span>
          </Link>
          <Link
            href="#"
            className={`hover:text-[#8BC34A] transition-colors ${scrolled ? "text-gray-600" : ""}`}
        ...
        <div className="flex items-center gap-3">
          <Link href="/login">
            <Button
              className={`px-5 py-2 bg-transparent rounded-full font-bold text-sm transition-all ${
                scrolled
