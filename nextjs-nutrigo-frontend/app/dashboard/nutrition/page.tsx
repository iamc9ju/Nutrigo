...
export default function NutritionistDetailPage() {
    ...
    const [nutritionist, setNutritionist] = useState<NutritionistDetail | null>(
        null,
    );
    ...
    const specialties = nutritionist.nutritionistSpecialties.map(
        (ns) => ns.specialty,
    );
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6">
                            ...
                            <div className="flex items-center gap-2 mb-4">
                                <div className="flex gap-0.5">
                                    {Array.from({ length: 5 }).map((_, i) => (
                                        <Star
                                            key={i}
                                            className={`w-5 h-5 ${i < Math.round(nutritionist.avgRating) ? "fill-yellow-400 text-yellow-400" : "text-gray-200"}`}
                                        />
                                    ))}
                                </div>
                                ...
                                <span className="text-sm text-[#8a7550]">
            ...
            {specialties.length > 0 && (
                <div className="bg-white rounded-[32px] p-8 border-2 border-[#f0e6cc] shadow-sm">
                    <h2 className="text-lg font-bold text-[#3d3522] mb-4">
                        ความเชี่ยวชาญ
                    </h2>
                    <div className="flex flex-wrap gap-3">
                        {specialties.map((s) => (
                            <span
                                key={s.id}
                                className="px-4 py-2 bg-[#C6E065]/20 text-[#4A6707] font-bold text-sm rounded-full border border-[#C6E065]/40"
                            >
                                {s.name}
                            </span>
                        ))}
                ...
                <div className="bg-white rounded-[32px] p-8 border-2 border-[#f0e6cc] shadow-sm">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {nutritionist.nutritionistSchedules.map((schedule, i) => (
                            <div
                                key={i}
                                className="flex items-center justify-between bg-[#faf8f2] rounded-xl px-5 py-3"
                            >
                                <span className="font-bold text-[#3d3522]">
                                    วัน{dayNames[schedule.dayOfWeek]}
                                </span>
                            ...
                            <div key={i} className="bg-[#faf8f2] rounded-2xl p-5">
                                ...
                                <div className="flex gap-0.5 mb-2">
                                    {Array.from({ length: 5 }).map((_, j) => (
                                        <Star
                                            key={j}
                                            className={`w-4 h-4 ${j < review.rating ? "fill-yellow-400 text-yellow-400" : "text-gray-200"}`}
                                        />
                                    ))}
                                </div>
                                {review.comment && (
