...
import { usePatientProfile, useUpdateHealthMetrics } from '@/hooks/usePatientProfile'; // Import hook
...
export default function HealthMetricsPage() {
    ...
    const getBMICategory = (score: number) => {
        if (score < 18.5) return { label: 'น้ำหนักน้อยเกินไป', color: 'text-blue-500', bg: 'bg-blue-50' };
        if (score < 23) return { label: 'น้ำหนักปกติ', color: 'text-[#C6E065]', bg: 'bg-[#C6E065]/10' };
        if (score < 25) return { label: 'น้ำหนักเกิน (ท้วม)', color: 'text-yellow-500', bg: 'bg-yellow-50' };
        if (score < 30) return { label: 'อ้วนระดับ 1', color: 'text-orange-500', bg: 'bg-orange-50' };
        ...
        updateMetricsMutation.mutate(
            ...
            },
            {
                onSuccess: () => {
                    ...
                    router.push('/dashboard/setting'); // แนะนำให้กลับไปหน้า Setting เพื่อดูผลลัพธ์
                },
                onError: (err: any) => {
                    Swal.fire({
                        icon: 'error',
        ...
        <div className="min-h-screen bg-[#faf8f2] p-6">
            <div className="max-w-md mx-auto">
                ...
                <div className="text-center mb-8">
                    <h1 className="text-3xl font-black text-[#3d3522] mb-2 uppercase tracking-tight">Health Metrics</h1>
                    <p className="text-[#8a7550] font-medium">บันทึกข้อมูลดัชนีมวลกายของคุณ</p>
                </div>
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    {/* Result Card (BMI) */}
                    <div className={`rounded-[32px] p-8 border-2 transition-all duration-500 ${bmi ? 'bg-white border-[#C6E065]/30' : 'bg-white/50 border-dashed border-[#f0e6cc]'
                        }`}>
                        {!bmi ? (
                            <div className="text-center py-4 flex flex-col items-center gap-3">
                                <Activity className="w-10 h-10 text-[#f0e6cc]" />
                                <p className="text-[#8a7550]/60 font-bold text-sm">กรอกข้อมูลเพื่อคำนวณค่า BMI</p>
                            </div>
                        ) : (
                            <div className="space-y-6 animate-in fade-in zoom-in duration-300">
                                <div className="flex justify-between items-end">
                                    <div>
                                        <p className="text-xs font-black text-[#8a7550] uppercase tracking-widest mb-1">Your Body Mass Index</p>
                                        <h2 className="text-5xl font-black text-[#3d3522]">{bmi}</h2>
                                    </div>
                                    <div className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-wider ${getBMICategory(bmi).bg} ${getBMICategory(bmi).color}`}>
                                ...
                                <div className="h-3 bg-[#faf8f2] rounded-full overflow-hidden flex shadow-inner">
                                    ...
                                    <div className="h-full bg-[#C6E065]" style={{ width: '4.5%' }} />
                                    <div className="h-full bg-yellow-400" style={{ width: '2%' }} />
