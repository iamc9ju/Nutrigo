...
export default function CompleteProfilePage() {
  ...
  const addCustomDisease = () => {
    if (
      customDisease.trim() &&
      !selectedDiseases.includes(customDisease.trim())
    ) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (selectedDiseases.length > 0)
        payload.chronicDiseases = selectedDiseases;
    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-[#faf8f2]">
      <div className="w-full max-w-lg">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-black text-[#3d3522] mb-2">
            สร้างโปรไฟล์สุขภาพ
          </h1>
          <p className="text-[#8a7550]">
            ตอบคำถามสั้นๆ เพื่อให้เรารู้จักคุณมากขึ้น
          </p>
        </div>
              ...
              <div className="space-y-8 animate-in fade-in slide-in-from-right-8 duration-500">
                <div className="text-center">
                  ...
                  <p className="text-[#8a7550] mt-1">
                    เราจะใช้ข้อมูลนี้เพื่อคำนวณแผนสุขภาพที่เหมาะสมกับวัย
                  </p>
                </div>
            ...
            {step === 2 && (
              <div className="space-y-8 animate-in fade-in slide-in-from-right-8 duration-500">
                <div className="text-center">
                  ...
                  <p className="text-[#8a7550] mt-1">
                    เพื่อการคำนวณปริมาณพลังงานที่แม่นยำ
                  </p>
                </div>
                <div className="grid grid-cols-1 gap-4">
                  {genderOptions.map((g) => (
                    ...
                    >
                      <div
                        className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl bg-white/50 shadow-sm
                                                ${gender === g.value ? "bg-white/80" : ""}`}
                      >
                        {g.emoji}
                      </div>
                      <span
                        className={`text-lg font-bold ${gender === g.value ? "text-[#3d3522]" : "text-[#8a7550]"}`}
                      >
                        {g.label}
                      </span>
                      {gender === g.value && (
            ...
            {step === 3 && (
              <div className="space-y-8 animate-in fade-in slide-in-from-right-8 duration-500">
                <div className="text-center">
                  ...
                  <p className="text-[#8a7550] mt-1">
                    ข้อมูลนี้ช่วยให้เราแนะนำอาหารได้ดียิ่งขึ้น
                  </p>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  {bloodTypeOptions.map((bt) => (
                    ...
                    >
                      <span
                        className={`text-4xl font-black mb-1 ${bloodType === bt ? "text-[#3d3522]" : "text-[#8a7550]"}`}
                      >
                        {bt}
                      </span>
                      <span
                        className={`text-xs font-bold uppercase tracking-wider ${bloodType === bt ? "text-[#3d3522]/80" : "text-[#8a7550]/60"}`}
                      >
                        Blood Type
                      </span>
                    </button>
            ...
            {step === 4 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-500">
                <div className="text-center">
                  ...
                  <p className="text-[#8a7550] mt-1">
                    เลือกรายการที่ตรงกับคุณ (เลือกได้มากกว่า 1)
                  </p>
                </div>
                <div className="max-h-[300px] overflow-y-auto pr-2 -mr-2 space-y-4">
                  ...
                  <div className="pt-4 border-t border-[#f0e6cc]">
                    <p className="text-sm font-bold text-[#3d3522] mb-3 ml-1">
                      อื่นๆ / ระบุเพิ่มเติม
                    </p>
                  ...
                  {selectedDiseases.length > 0 && (
                    <div className="bg-[#faf8f2] rounded-2xl p-4 border border-[#f0e6cc]">
                      <p className="text-xs text-[#8a7550] mb-2 font-bold uppercase tracking-wider">
                        รายการที่เลือก:
                      </p>
